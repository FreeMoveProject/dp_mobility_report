...
from dp_mobility_report.report.html.html_utils import fmt, get_template, fmt_config
...
def render_config(dpmreport: "DpMobilityReport") -> str:
    ...
    if (dpmreport.tessellation is None):
        args["tessellation_info"] = "No tessellation has been provided. All analyses based on the tessellation have been excluded."

    if not pd.core.dtypes.common.is_datetime64_dtype(dpmreport.df[const.DATETIME]):
        args["timestamp_info"] = "Dataframe does not contain timestamps. All analyses based on timestamps have been excluded."

    if  max(dpmreport.df[const.TID].value_counts()) == 1:
        args["od_info"] = "All trips in the dataset only contain a single record, therefore, all origin-destination analyses (OD Flows, travel time, jump length) have been excluded."
...
def render_benchmark_config(benchmarkreport: "BenchmarkReport") -> str:
    ...
    if (not pd.core.dtypes.common.is_datetime64_dtype(benchmarkreport.report_base.df[const.DATETIME])) or (not pd.core.dtypes.common.is_datetime64_dtype(benchmarkreport.report_alternative.df[const.DATETIME])):
        args["timestamp_info"] = "At least one of the datasets does not contain timestamps. All analyses based on timestamps have been excluded."
    ...
    if  (max(benchmarkreport.report_base.df[const.TID].value_counts()) == 1) or (max(benchmarkreport.report_alternative.df[const.TID].value_counts()) == 1):
        args["od_info"] = "All trips in at least one of the datasets only contain a single record, therefore, all origin-destination analyses (OD Flows, travel time, jump length) have been excluded."
