...
def test_quartiles_dp():
    ...
    array = pd.Series([np.datetime64('2021-07-01'), np.datetime64('2021-07-03'), np.datetime64('2021-07-03')])
    quartiles = diff_privacy.quartiles_dp(array, eps = 0.1, sensitivity=1)
    assert quartiles.min() >= array.min()
    assert quartiles.max() <= array.max()
    assert np.issubdtype(quartiles.dtype, np.datetime64)

    array = pd.Series([np.timedelta64(1, 'D'), np.timedelta64(1, 'D'), np.timedelta64(10, 'h')])
    quartiles = diff_privacy.quartiles_dp(array, eps = 0.1, sensitivity=1)
    assert quartiles.min() >= array.min()
    assert quartiles.max() <= array.max()
...
def test_counts_dp():
    ...
    dp_counts = diff_privacy.counts_dp(counts, eps = None, sensitivity = None, nonzero = False)
