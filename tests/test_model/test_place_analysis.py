...
def test_get_visits_per_tile(test_mreport):
    """Correct visits per tile values without noise."""
    visits_per_location = place_analysis.get_visits_per_tile(test_mreport, None, None)
...
def test_get_visits_per_tile_timewindow(test_mreport):
    visits_timewindow = place_analysis.get_visits_per_tile_timewindow(
        test_mreport, None, None
    ).data
